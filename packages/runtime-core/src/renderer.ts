...
function baseCreateRenderer(
  ...
  const patchElement = (
    ...
    if (patchFlag > 0) {
      // the presence of a patchFlag means this element's render code was patchFlag 的存在意味着该元素的渲染代码由编译器生成，可走快速路径。
      // generated by the compiler and can take the fast path.
      // in this path old node and new node are guaranteed to have the same shape 在此路径下，旧节点和新节点保证具有相同的结构（即位于源模板中的同一位置）
  ...
  const patchKeyedChildren = (
    ...
    // 总结:首首 -> 尾尾 -> 再通过"最长递增子序列"的方式进行对比
    // 1. sync from start 去掉相同的首
    // (a b) c
    // (a b) d e
    while (i <= e1 && i <= e2) {
      const n1 = c1[i]
